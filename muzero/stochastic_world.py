...
def load_env_config(filename): # the main function is called from the root directory, so we have to reference the muzero child directory
    with open(f"env_configs/{filename}.pkl",'rb') as file: return pickle.load(file)
    

if __name__ == '__main__':
    config = { 'env': { 'num_states': 4,
                        'num_actions': 3,
                        'timestep_limit': 10,
